...
- **Soft FKs (documented no-FK scalars):** `User.supplierId`, `Approval.entityType/entityId`, `WalletAccount.ledgerAccountId`, `InventoryItem.materialId/supplierId`, `Milestone.phaseId`, `VariationOrder.phaseId`, `SitePhoto.zoneId`, `PhotoHash.photoId/packId`, `AiInsight.targetId`, `PaymentRequest.paidTxnId`, `Transaction.ledgerTxnId` (comment L264 claims "unique per txn" but there is **no** `@unique`). → DB-8/DB-9.
  > **UPDATE (2026-09-25, task 6-b / #127 — DB-9 CLOSED):** the soft-FK sweep landed a
  > per-column disposition with a central registry — **ADR 0010
  > (`docs/adr/0010-soft-fk-registry.md`)**, the inventory of record. Summary:
  > `Transaction.ledgerTxnId` **CONSTRAINED** (migration `21_transaction_ledger_txn_unique`
  > — the "unique per txn" comment is now DB-enforced TRUE; NULLs unaffected,
  > SQLite unique indexes skip them; a pre-existing dup corpus fails the
  > migration loudly, the migration-10 precedent); `Milestone.phaseId`,
  > `VariationOrder.phaseId`, `SitePhoto.zoneId` **VALIDATED** (pre-existing write-time
  > seams — `resolvePhase` refuses foreign phases in-project, `photo.zone`
  > refuses foreign zones + `zone.delete` untags first); `InventoryItem.supplierId`
  > **VALIDATED, NEW** (the `upsertItem` write seam rejects unknown ids inside the
  > movement `$transaction`); everything else **documented keep-soft** with named
  > guards (e.g. `User.supplierId` → `guard.ts pinnedSupplierId()` 403 "no supplier
  > linked"). The sweep also found ~12 columns beyond the audit's list
  > (`EscrowWallet.ledgerAccountId`, `PaymentRequest.relatedEntityType/Id`,
  > `DrawPack.ledgerTxnId/ledgerRef`, `LedgerTransaction.reversalOfId`,
  > `LedgerAccount.ownerId`, `WalletAccount.ownerId`, `AuditEvent.entity/entityId`,
  > `Attachment.entityType/entityId`, `AiInsight.packId`, `User.projectId`,
  > the `evidencePhotoIds` JSON arrays, `StockMovement.reference`) — all in the
  > registry with decisions. One parity divergence recorded: the Supabase design
  > declares `ledger_transactions.reversal_of_id` a real self-FK while SQLite keeps
  > it soft (a real FK needs a table rebuild — not additive).
...
| DB-8 | Medium | Business codes not unique: requestCode/orderCode/invoiceCode/requestCode; `Transaction.ledgerTxnId` comment claims uniqueness it doesn't have | schema L657, L749, L865, L1040, L264; design doc §10.1 admits it | "Promote business codes to unique (data audit first) — MR-/PO-/INV-/PR- + Transaction.ledgerTxnId" — **the PO/invoice code half CLOSED by #121 (migration 10: UNIQUE (projectId, orderCode)/(projectId, invoiceCode), deliberate-not-unique requestCode documented); the `Transaction.ledgerTxnId` half CLOSED 2026-09-25 via #127 (migration 21, see DB-9/ADR 0010)** |
| DB-9 | Low-Medium | 11+ soft FK columns can dangle (mostly fail-closed in code, documented) | schema L447, L700–701, L1031, L1071, L320, L341, L225, L1282–1288 | "Harden soft FK links (supplierId, approval entity, wallet ledger link) with validation or constraints" — **CLOSED 2026-09-25 via #127: per-column disposition (constrain / validate / documented-keep-soft) recorded in ADR 0010, the soft-FK registry; `Transaction.ledgerTxnId` unique via migration 21; `InventoryItem.supplierId` validated at the `upsertItem` write seam; every other column keep-soft with a named guard; sweep found ~12 more columns beyond the audit's list, all registered** |
